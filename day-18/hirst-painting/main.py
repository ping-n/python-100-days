###This code will not work in repl.it as there is no access to the colorgram package here.###
##We talk about this in the video tutorials##
# import colorgram

# rgb_colors = []
# colors = colorgram.extract('image.jpg', 30)
# for color in colors:
#     r = color.rgb.r
#     g = color.rgb.g
#     b = color.rgb.b
#     new_color = (r, g, b)
#     rgb_colors.append(new_color)

import turtle as t
import random

color_list = [(240, 245, 241), (202, 164, 109), (238, 240, 245), (150, 75, 49), (223, 201, 135), (52, 93, 124),
              (172, 154, 40),
              (140, 30, 19), (133, 163, 185), (198, 91, 71), (46, 122, 86), (72, 43, 35), (145, 178, 148), (13, 99, 71),
              (233, 175, 164), (161, 142, 158), (105, 74, 77), (55, 46, 50), (183, 205, 171), (36, 60, 74),
              (18, 86, 90),
              (81, 148, 129), (148, 17, 20), (14, 70, 64), (30, 68, 100), (107, 127, 153), (174, 94, 97),
              (176, 192, 209)]

# Set up colormode to rgb
t.colormode(255)
# Set up turtle
tim = t.Turtle()
tim.ht()
tim.penup()
tim.speed(100)


# draw the circle (number of dots)
def draw_dot(num_dot):
    for _ in range(num_dot):
        tim.dot(20, random.choice(color_list))
        tim.forward(50)


# reset direct of turtle
def reset_direction(num_dot):
    tim.left(90)
    tim.forward(50)
    tim.seth(180)
    tim.forward(num_dot * 50)
    tim.seth(0)


# starting the turtle
def starting_point():
    tim.seth(225)
    tim.forward(300)
    tim.seth(0)


# paint the dots with number of loops and number of dots per loop
def paint_dots(loop, num_dot):
    starting_point()
    for _ in range(loop):
        draw_dot(num_dot)
        reset_direction(num_dot)


if __name__ == "__main__":
    paint_dots(10, 10)
    t.screensize(200, 200)
    t.exitonclick()
